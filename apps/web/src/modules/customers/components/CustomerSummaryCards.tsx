...
export const CustomerSummaryCards: React.FC<CustomerSummaryCardsProps> = ({
  ...
  const getPoints = (val: number) => (val > 0 ? [val, val] : [0, 0]);
  ...
  const cards = [
    {
      ...
      dataPoints: getPoints(totalCustomers),
    },
    {
      id: 'active-customers',
      title: 'ACTIVE CUSTOMERS',
      metric: activeCustomers.toLocaleString('en-IN'),
      supporting: totalCustomers > 0 ? `${Math.round((activeCustomers / totalCustomers) * 100)}% of total` : '0% of total',
      supportingClass: 'text-emerald-700 font-semibold',
      icon: <UserCheck className="w-5 h-5 text-white" />,
      iconBg: 'bg-emerald-600',
      curveColor: 'green' as const,
      dataPoints: getPoints(activeCustomers),
    },
    {
      id: 'new-this-month',
      title: 'NEW THIS MONTH',
      metric: newThisMonth.toLocaleString('en-IN'),
      supporting: totalCustomers > 0 ? `${Math.round((newThisMonth / totalCustomers) * 100)}% of total` : 'Registered this month',
      supportingClass: 'text-teal-700 font-semibold',
      icon: <UserPlus className="w-5 h-5 text-white" />,
      iconBg: 'bg-teal-600',
      curveColor: 'purple' as const,
      dataPoints: getPoints(newThisMonth),
    },
    {
      id: 'active-warranty',
      title: 'WITH ACTIVE WARRANTY',
      metric: withWarranty.toLocaleString('en-IN'),
      supporting: totalCustomers > 0 ? `${Math.round((withWarranty / totalCustomers) * 100)}% of total base` : '0% of total base',
      supportingClass: 'text-slate-600 font-medium',
      icon: <ShieldCheck className="w-5 h-5 text-white" />,
      iconBg: 'bg-amber-600',
      curveColor: 'orange' as const,
      dataPoints: getPoints(withWarranty),
    },
    {
      id: 'due-for-service',
      title: 'DUE FOR SERVICE',
      metric: dueForService.toLocaleString('en-IN'),
      supporting: dueForService > 0 ? 'Urgent attention required' : 'No service due',
      supportingClass: dueForService > 0 ? 'text-red-700 font-semibold' : 'text-slate-500 font-medium',
      icon: <Calendar className="w-5 h-5 text-white" />,
      iconBg: 'bg-red-600',
      curveColor: 'red' as const,
      dataPoints: getPoints(dueForService),
    },
  ];
